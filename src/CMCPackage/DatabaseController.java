...
/**
 * This class is the DatabaseController class--the one that will be getting our
 * data from our database. For any method that is called by the controller
 * classes that needs information from the database, DatabaseController will
 * retrieve that information and pass it along to the desired controller
 * 
 * @author jcblomquist
 * @version 3/10/2016
 *
...
public class DatabaseController {
	...
	/**
	 * the database library that will be used to retrieve, add, remove, and
	 * update information
	 */
	...
	/**
	 * addSchool allows a new school to be added to the UniversityDBLibrary. It
	 * first checks to see if there is already a school with that name. If there
	 * is, it returns an int value of -1, implying that the school was not
	 * added.
	 * 
	 * @param name
	 * @param state
	 ...
	 */
	public int addSchool(String name, String state, String location, String control, int numStudents, double perFemale,
			double satVerbal, double satMath, double expenses, double perFinancial, int numApplicants,
			double perAdmitted, double perEnrolled, int acadScale, int socialScale, int QOLScale) {
		if (!(state.equals("ALABAMA") || state.equals("ALASKA") || state.equals("ARIZONA") || state.equals("ARKANSAS")
				...
				|| state.equals("RHODE ISLAND") || state.equals("SOUTH CAROLINA") || state.equals("SOUTH DAKOTA")
				|| state.equals("TENNESSEE") || state.equals("TEXAS") || state.equals("UTAH") || state.equals("VERMONT")
				|| state.equals("VIRGINIA") || state.equals("WASHINGTON") || state.equals("WEST VIRGINIA")
				|| state.equals("WISCONSIN") || state.equals("WYOMING") || state.equals("DISTRICT OF COLUMBIA")
				|| state.equals("FOREIGN") || state.equals("-1")))
			return -1;
		...
		if ((QOLScale > 5) || ((QOLScale < 1) && (QOLScale != -1)))
			return -1;
		return databaseLibrary.university_addUniversity(name, state, location, control, numStudents, perFemale,
				satVerbal, satMath, expenses, perFinancial, numApplicants, perAdmitted, perEnrolled, acadScale,
				socialScale, QOLScale);
	}

	/**
	 * editSchool checks to see that a school exists within the database, then
	 * updates its information when it is found. The parameters that will be
	 * updated are given by the user.
	 * 
	 * @param name
	 * @param state
	 * @param location
	 ...
	 * @return 1 if successful, -1 if unsuccessful
	 */
	public int editSchool(String name, String state, String location, String control, int numStudents, double perFemale,
			double satVerbal, double satMath, double expenses, double perFinancial, int numApplicants,
			double perAdmitted, double perEnrolled, int acadScale, int socialScale, int QOLScale) {
	...
	/**
	 * addNewMember adds a new member to the CMC system. It checks to see if
	 * there is already a member with the desired username- if there is not, the
	 * new member is added to the system.
	 * 
	 * @param firstName
	 * @param lastName
	 * @param username
	...
	/**
	 * editUser takes information for as user as a parameter from an admin and
	 * attempts to update the information. It first searches for a user with the
	 * username, then updates the other information. It searches for the
	 * username because it is uneditable by the admin.
	 *
	 * @param firstName
	 * @param lastName
	 * @param username
	...
	/**
	 * getSchools fetches all of the schools stored in the database and provides
	 * them to the user
	 * 
	 * @return all of the users in the library
	 */
	...
	/**
	 * findByName fetches the information for a user based on a userName
	 * parameter
	 * 
	 * @param user
	 *            representing userName to search for
	...
	/**
	 * getSchoolsWithEmphases fetches the information for all schools and their
	 * emphases
	 * 
	 * @param user
	 *            representing userName to search for
	...
	/**
	 * saveSchool calls the user_saveSchool method from the UniversityDBLibrary
	 * class
	 * 
	 * @param userName
	 *            of the agent
	 * @param school
	 *            name
	 * @return int representing number of schools added, -1 if the operation
	 *         failed
	 */
	...
	/**
	 * removeSchool calls the user_removeSchool method from the
	 * UniversityDBLibrary class
	 * 
	 * @param userName
	 *            of the agent
	 * @param school
	 *            name
	 * @return int representing number of schools removed, -1 if the operation
	 *         failed
	 */
	...
	/**
	 * addUniversityEmphasis adds an emphasis, paired with a school, to the
	 * database
	 * 
	 * @param school
	 * @param emphasis
	 ...
	 * saved schools from the database
	 * 
